...
function auth50():array{if(empty($_SESSION['user']))r50(401,['ok'=>false,'message'=>'Belum login']);return $_SESSION['user'];}
function finance50():array{$u=auth50();if(($u['role']??'')!=='finance')r50(403,['ok'=>false,'message'=>'Akses khusus Finance/Bendahara']);return$u;}
function parent50():array{$u=auth50();if(($u['role']??'')!=='parent')r50(403,['ok'=>false,'message'=>'Akses khusus Orang Tua/Wali']);return$u;}
...
function createPayment50(PDO $pdo,int $schoolId,array $u,array $bill,string $method):array{
  ...
  $receipt=nextReceipt50($pdo,$schoolId);$external='server:'.$receipt;
...
if($path==='/api/v50/health'&&$method==='GET')r50(200,['ok'=>true,'version'=>'5.0','finance_server_first'=>true,'atomic_payments'=>true,'server_receipts'=>true]);

if($path==='/api/v50/parent/payments'&&$method==='GET'){
  $u=parent50();$studentId=(int)($_GET['student_id']??0);
  if($studentId<=0)r50(422,['ok'=>false,'message'=>'Siswa belum dipilih']);
  $q=$pdo->prepare('SELECT 1 FROM guardian_students gs JOIN students s ON s.id=gs.student_id WHERE gs.guardian_user_id=? AND s.school_id=? AND s.id=?');$q->execute([$u['id'],$schoolId,$studentId]);
  if(!$q->fetchColumn())r50(403,['ok'=>false,'message'=>'Siswa tidak terhubung dengan akun wali']);
  $q=$pdo->prepare("SELECT p.id,p.receipt,p.amount,p.method,p.paid_at,p.voided,b.title,u.name verified_by
    FROM payments p JOIN bills b ON b.id=p.bill_id LEFT JOIN users u ON u.id=p.verified_by
    WHERE p.school_id=? AND p.student_id=? ORDER BY p.paid_at DESC,p.id DESC");
  $q->execute([$schoolId,$studentId]);$rows=[];
  foreach($q->fetchAll() as $p)$rows[]=['id'=>(int)$p['id'],'receipt'=>$p['receipt'],'amount'=>(float)$p['amount'],'method'=>$p['method'],'paidAt'=>$p['paid_at'],'voided'=>(bool)$p['voided'],'title'=>$p['title'],'verifiedBy'=>$p['verified_by']??''];
  r50(200,['ok'=>true,'payments'=>$rows,'serverTime'=>date(DATE_ATOM)]);
}
...
if(preg_match('#^/api/v50/finance/bills/(\d+)/approve$#',$path,$m)&&$method==='POST'){
  $u=finance50();$billId=(int)$m[1];$pdo->beginTransaction();
  try{$bill=billLock50($pdo,$schoolId,$billId);if($bill['status']!=='pending')r50(409,['ok'=>false,'message'=>'Tagihan tidak lagi menunggu verifikasi']);if(trim((string)$bill['proof_name'])==='')r50(409,['ok'=>false,'message'=>'Bukti transfer belum tersedia']);$payment=createPayment50($pdo,$schoolId,$u,$bill,'Transfer');$pdo->commit();r50(200,['ok'=>true,'message'=>'Bukti diterima dan pembayaran dilunaskan','payment'=>$payment]);}
  catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();if($e instanceof PDOException)r50(500,['ok'=>false,'message'=>'Transaksi database gagal']);throw$e;}
}

if(preg_match('#^/api/v50/finance/bills/(\d+)/reject$#',$path,$m)&&$method==='POST'){
  $u=finance50();$in=input50();$reason=trim((string)($in['reason']??''));$billId=(int)$m[1];if(mb_strlen($reason)<3)r50(422,['ok'=>false,'message'=>'Alasan penolakan wajib diisi minimal 3 karakter']);$pdo->beginTransaction();
  try{$bill=billLock50($pdo,$schoolId,$billId);if($bill['status']!=='pending')r50(409,['ok'=>false,'message'=>'Tagihan tidak lagi menunggu verifikasi']);$pdo->prepare("UPDATE bills SET status='unpaid',payment_method=NULL,proof_name=NULL,updated_at=NOW() WHERE id=?")->execute([$billId]);audit50($pdo,$schoolId,(int)$u['id'],'proof.rejected','bill',(string)$billId,['reason'=>$reason,'old_proof'=>$bill['proof_name']]);notifyStudent50($pdo,$schoolId,(int)$bill['student_id'],'warning','Bukti pembayaran ditolak','Bukti untuk '.$bill['title'].' ditolak. Alasan: '.$reason,'bill',(string)$billId);$pdo->commit();r50(200,['ok'=>true,'message'=>'Bukti pembayaran ditolak']);}
  catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();if($e instanceof PDOException)r50(500,['ok'=>false,'message'=>'Transaksi database gagal']);throw$e;}
}

if(preg_match('#^/api/v50/finance/payments/(\d+)/void$#',$path,$m)&&$method==='POST'){
  $u=finance50();$in=input50();$reason=trim((string)($in['reason']??''));$paymentId=(int)$m[1];if(mb_strlen($reason)<5)r50(422,['ok'=>false,'message'=>'Alasan pembatalan wajib diisi minimal 5 karakter']);$pdo->beginTransaction();
  try{$q=$pdo->prepare('SELECT * FROM payments WHERE school_id=? AND id=? FOR UPDATE');$q->execute([$schoolId,$paymentId]);$p=$q->fetch();if(!$p)r50(404,['ok'=>false,'message'=>'Pembayaran tidak ditemukan']);if((bool)$p['voided'])r50(409,['ok'=>false,'message'=>'Pembayaran sudah dibatalkan sebelumnya']);$bill=billLock50($pdo,$schoolId,(int)$p['bill_id']);$pdo->prepare('UPDATE payments SET voided=TRUE,voided_at=NOW(),void_reason=?,voided_by=? WHERE id=?')->execute([$reason,$u['id'],$paymentId]);$pdo->prepare("UPDATE bills SET status='unpaid',payment_method=NULL,proof_name=NULL,updated_at=NOW() WHERE id=?")->execute([$bill['id']]);audit50($pdo,$schoolId,(int)$u['id'],'payment.voided','payment',(string)$paymentId,['receipt'=>$p['receipt'],'reason'=>$reason,'bill_id'=>(int)$bill['id']]);notifyStudent50($pdo,$schoolId,(int)$bill['student_id'],'warning','Pembayaran dibatalkan','Pembayaran '.$bill['title'].' dengan kwitansi '.$p['receipt'].' dibatalkan oleh sekolah. Silakan hubungi petugas bila diperlukan.','payment',(string)$paymentId);$pdo->commit();r50(200,['ok'=>true,'message'=>'Pembayaran berhasil dibatalkan','receipt'=>$p['receipt']]);}
  catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();if($e instanceof PDOException)r50(500,['ok'=>false,'message'=>'Transaksi database gagal']);throw$e;}
}
