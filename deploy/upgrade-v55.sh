...
[ -f "$APP_DIR/backend/v1.php" ] || { echo 'ERROR: backend/v1.php tidak ditemukan' >&2; exit 1; }
[ -f "$APP_DIR/backend/v55.php" ] || { echo 'ERROR: backend/v55.php tidak ditemukan' >&2; exit 1; }
[ -f "$MIGRATION" ] || { echo 'ERROR: migration V5.5 tidak ditemukan' >&2; exit 1; }
[ -f "$APP_DIR/reports-scale-v55.js" ] || { echo 'ERROR: frontend V5.5 tidak ditemukan' >&2; exit 1; }
...
command -v node >/dev/null 2>&1 && node --check "$APP_DIR/reports-scale-v55.js" || echo 'INFO: node tidak tersedia; JS syntax check dilewati.'
