...
printf '[1/9] Install PostgreSQL, PHP-FPM, PDO PostgreSQL...\n'
...
command -v psql >/dev/null 2>&1 || { echo 'ERROR: psql tidak ditemukan setelah instalasi.' >&2; exit 1; }
command -v php >/dev/null 2>&1 || { echo 'ERROR: PHP tidak ditemukan setelah instalasi.' >&2; exit 1; }
PHP_VER="$(php -r 'echo PHP_MAJOR_VERSION.".".PHP_MINOR_VERSION;')"
PHP_SOCK="/run/php/php${PHP_VER}-fpm.sock"

printf '[2/9] Start PostgreSQL and create database/user...\n'
...
printf '[6/9] Configure base Nginx API route...\n'
TMP_NGINX="$(mktemp)"
cat >"$TMP_NGINX" <<NGINX
location ^~ /api/ {
...
[ -f "$SITE" ] || { echo "ERROR: Konfigurasi Nginx EduPay tidak ditemukan di $SITE" >&2; exit 1; }
...
printf '[7/9] Enable PHP-FPM and test Nginx...\n'
as_root systemctl enable --now "php${PHP_VER}-fpm"
[ -S "$PHP_SOCK" ] || { echo "ERROR: PHP-FPM socket tidak ditemukan: $PHP_SOCK" >&2; exit 1; }
php -l "$APP_DIR/backend/api.php"
as_root nginx -t
as_root systemctl reload nginx

printf '[8/9] API health check...\n'
sleep 2
HEALTH="$(curl -fsS "https://${DOMAIN}/api/health" || true)"
printf '%s\n' "$HEALTH"
if ! printf '%s' "$HEALTH" | grep -q '"ok"'; then echo 'WARNING: API health belum mengembalikan JSON ok=true.' >&2; fi

printf '[9/9] Backend installed.\n'
printf '\nIMPORTANT - save these values securely now:\n'
printf 'Database user     : %s\n' "$DB_USER"
printf 'Database password : %s\n' "$DB_PASSWORD"
printf 'Bootstrap key     : %s\n' "$BOOTSTRAP_KEY"
printf '\nConfig stored at: %s\n' "$CONFIG_FILE"
printf 'Next step: bootstrap admin/finance, then run latest upgrade script.\n'
