...
PKGS=(nginx postgresql postgresql-client php-cli php-fpm php-pgsql php-mbstring php-zip php-curl openssl curl ca-certificates)
[ "$ENABLE_SSL" = 1 ] && PKGS+=(certbot python3-certbot-nginx)
as_root env DEBIAN_FRONTEND=noninteractive apt-get install -y "${PKGS[@]}"
command -v php >/dev/null || fail 'PHP gagal dipasang'
command -v openssl >/dev/null || fail 'OpenSSL gagal dipasang'
...
TMP_CONFIG="$(mktemp)"
EDUPAY_CFG_DOMAIN="$DOMAIN" EDUPAY_CFG_DB_NAME="$DB_NAME" EDUPAY_CFG_DB_USER="$DB_USER" EDUPAY_CFG_DB_PASSWORD="$DB_PASSWORD" EDUPAY_CFG_SCHOOL_CODE="$SCHOOL_CODE" EDUPAY_CFG_SCHOOL_NAME="$SCHOOL_NAME" EDUPAY_CFG_BOOTSTRAP="$BOOTSTRAP_KEY" \
php -r '$c=["db"=>["dsn"=>"pgsql:host=127.0.0.1;port=5432;dbname=".getenv("EDUPAY_CFG_DB_NAME"),"user"=>getenv("EDUPAY_CFG_DB_USER"),"password"=>getenv("EDUPAY_CFG_DB_PASSWORD")],"app"=>["base_url"=>"https://".getenv("EDUPAY_CFG_DOMAIN"),"cookie_name"=>"edupay_session","session_ttl"=>43200,"school_code"=>getenv("EDUPAY_CFG_SCHOOL_CODE"),"school_name"=>getenv("EDUPAY_CFG_SCHOOL_NAME"),"bootstrap_key"=>getenv("EDUPAY_CFG_BOOTSTRAP")]];echo "<?php\nreturn ".var_export($c,true).";\n";' > "$TMP_CONFIG"
...
as_root chmod +x "$APP_DIR/deploy/backup-edupay.sh" "$APP_DIR/deploy/verify-restore.sh" "$APP_DIR/deploy/final-uat-v56.sh" "$APP_DIR/deploy/release-gate-v60.sh" 2>/dev/null || true
...
if [ "$ENABLE_SSL" = 1 ]; then
  ...
  echo 'WARNING: SSL dilewati. Secure-cookie login tidak boleh digunakan production sebelum HTTPS aktif.'
fi
...
BASE="https://${DOMAIN}"; [ "$ENABLE_SSL" = 1 ] || BASE="http://${DOMAIN}"
sleep 2
...
printf 'Database         : %s\nDatabase user    : %s\n' "$DB_NAME" "$DB_USER"
printf 'DB password      : %s\nBootstrap key    : %s\n' "$DB_PASSWORD" "$BOOTSTRAP_KEY"
printf '\nSimpan credential server secara aman dan jangan commit backend/config.php.\n'
