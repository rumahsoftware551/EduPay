...
for F in backend/v1.php backend/v56.php backend/v56finance.php backend/v56scale.php backend/v552.php backend/v55.php commercial-final-v56.js commercial-final-v56.css deploy/backup-edupay.sh deploy/verify-restore.sh deploy/systemd/edupay-backup.service deploy/systemd/edupay-backup.timer index.html sw.js; do [ -f "$APP_DIR/$F" ] || { echo "ERROR: $F tidak ditemukan" >&2; exit 1; }; done
...
SELECT id,code,name,receipt_prefix,backup_retention_days
FROM schools
...
sudo chown root:www-data /var/lib/edupay/maintenance
sudo chmod 750 /var/lib/edupay/proofs /var/lib/edupay/branding /var/lib/edupay/maintenance
sudo touch /var/log/edupay/app.log
sudo chown www-data:www-data /var/log/edupay/app.log
sudo chmod 640 /var/log/edupay/app.log
...
php -l "$APP_DIR/backend/v56scale.php" >/dev/null
...
BRAND="$(curl -fsS https://edupay.rumahsoftware.site/api/v1/branding)"
echo "$BRAND"
