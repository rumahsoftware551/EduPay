...
STATUS_FILE="$MAINT_DIR/restore-status.json"
...
EOF
  sudo mv "$STATUS_TMP" "$STATUS_FILE"; sudo chmod 640 "$STATUS_FILE"; exit "$code"
}
trap fail_status ERR
trap cleanup EXIT

if [ -f "$BACKUP_DIR/manifest.sha256" ]; then (cd "$BACKUP_DIR" && sha256sum -c manifest.sha256); fi
tar -tzf "$BACKUP_DIR/proofs.tar.gz" >/dev/null
tar -tzf "$BACKUP_DIR/branding.tar.gz" >/dev/null
...
EOF
sudo mv "$STATUS_TMP" "$STATUS_FILE"; sudo chmod 640 "$STATUS_FILE"
trap - ERR
cleanup
trap - EXIT
