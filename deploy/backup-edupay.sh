...
DB_BYTES="$(sudo stat -c '%s' "$RUN_DIR/database.sql.gz")"; PROOF_BYTES="$(sudo stat -c '%s' "$RUN_DIR/proofs.tar.gz")"; BRAND_BYTES="$(sudo stat -c '%s' "$RUN_DIR/branding.tar.gz")"; FINISHED="$(date --iso-8601=seconds)"
